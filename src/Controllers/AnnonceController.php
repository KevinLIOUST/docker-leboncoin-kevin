...
class AnnonceController
{
    ...
    public function supprimerAnnonce()
    {
        ...
        if ($annonce->findById($id)["u_id"] == $_SESSION["user"]["id"]) {
            $image = $annonce->findImage($id)["a_picture"];
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (isset($_POST["description"])) {
                ...
                if (empty($_POST["description"])) {
                    ...
                    $errors["description"] = "500 caractères ou moins.";
                } else {
            ...
            if (empty($errors)) {
                ...
                if (!empty($_FILES['file']['name'])) {
                    ...
                    if (move_uploaded_file($tmpNameFichier, $destinationPath)) {
                        $reussi["fichierDeplace"] = "Le fichier a été déplacé avec succès vers : $destinationPath";
                    } else {
                        $reussi["fichierPasDeplace"] = "Erreur : Impossible de déplacer le fichier.";
                    }
                }
    ...
    public function modify()
    {
        ...
        if ($_SESSION["user"]["id"] == $infosAnnonce["u_id"]) {
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                ...
                if (empty($errors)) {
                    $imageId = $annonce->findImage($id)["a_picture"];
                    ...
                    if (!empty($_FILES['file']['name'])) {
                        ...
                        if (move_uploaded_file($tmpNameFichier, $destinationPath)) {
                            ...
                            $errors["fichierPasDeplace"] = "Erreur : Impossible de déplacer le fichier.";
                        }
                    }
            ...
            include_once __DIR__ . "/../Views/page404.php";
        }
    }
